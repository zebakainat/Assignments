    ...
    if x == 1:
        ...
        if Row_number in range(1, Row+1) and Column_number in range(1, Seats+1):
            if table_of_chart[str(Row_number-1)][str(Column_number)] == 'S':
                if Row*Seats > 60 and Row_number>int(Row/2):
                    prize_of_ticket = 8
                else:
                    prize_of_ticket = 10
                print('price_of_ticket - ', '$', prize_of_ticket)
                ...
                if conform == 'yes':
                    ...
                    Booked_seat += 1
                    current_income = current_income + prize_of_ticket

                    if Row*Seats>60:
                        Total_Income = int(Row/2)*Seats*10 + (Row-int(Row/2))*Seats*8
                    else:
        ...
        if Enter_row in range(1, Row+1) and Enter_column in range(1, Seats+1):
            if table_of_chart[str(Enter_row-1)][str(Enter_column)] == 'B':
                ...
                print('Ticket Price - ', '$', person['Ticket_prize'])
            else:
                print()
                print('---*---  Vacant seat  ---*---')
        else:
